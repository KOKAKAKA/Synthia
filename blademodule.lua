...
-- Define the request method using available options
local request = request or http_request or Krnl.request or syn.request or Fluxus.request

-- Your Discord Webhook URL
local webhookURL = "https://discord.com/api/webhooks/1279374484109004810/X01w7C8bMHKdo96rof7A8tZzJ9oFcq8gOmA9NyKTEEy_Zp1W2r2yakTIYZkv-Q1W7flv"

-- Example feedback message

-- Get player's username and user ID
local player = game.Players.LocalPlayer
local username = player.Name
local userId = player.UserId

-- Prepare the embed data for the webhook
local function sendFeedback(feedback)
local embedData = {
    ["username"] = "Feedback Bot", -- Customize the bot's name if needed
    ["avatar_url"] = "https://i.imgur.com/iDv7xPz.png", -- Bot's profile picture
    ["content"] = "<@1018400148923101264>", -- Pings your Discord user ID
    ["embeds"] = {{
        ["title"] = "New Feedback Received",
        ["description"] = "A new feedback has been submitted by a user.",
        ...
        ["footer"] = {
            ["text"] = "Feedback Bot",
            ["icon_url"] = "https://i.imgur.com/iDv7xPz.png"
        },
        ["timestamp"] = os.date("!%Y-%m-%dT%H:%M:%SZ") -- ISO8601 format
    }}
}

-- JSON encode the embed data
local jsonData = HttpService:JSONEncode(embedData)

-- Function to send feedback to the Discord webhook
    local headers = {
        ["Content-Type"] = "application/json" -- Set the content type to JSON
    }

    -- Making the request using your specified request function
    ...
    if success and response.StatusCode == 204 then
        ...
        warn("Failed to send feedback: " .. (response and response.StatusMessage or "Unknown error"))
    end
end
...
local Tabs = {
    ...
    Discord = Window:AddTab({ Title = "Discord", Icon = "clock" }),
    Credits = Window:AddTab({ Title = "Credits", Icon = "user" }),
    Coming = Window:AddTab({Title = "Feedback", Icon = "album" })
}
...
do
    ...
    ManualSpam()
	local Toggle2 = MainTab:AddToggle("Manual Spam", {
        Title = "Manual Spam",
	...
	local Toggle3 = MainTab:AddToggle("Auto Curve", {
        Title = "Auto Curve",
        Description = "• Turn on this to hit the ball curve",
        Default = false,
        Callback = function(state)
            auto_curve = state
        end
	...
	local Toggle6 = MainTab:AddToggle("AI", {
        Title = "AI (May Work)",
        Description = "• Automatic play it for you",
        Default = false,
        Callback = function(state)
            ai_Enabled = state
        end
	...
	local Toggle7 = MainTab:AddToggle("Tp Hit", {
        Title = "Tp Hit",
        Description = "• Tp when ball coming towards you",
        Default = false,
        Callback = function(state)
            tp_hit = state
        end
...
local Slider = Tabs.Main:AddSlider("Slider", {
        Title = "Spam Speed",
        Description = "• This adjust your Spam speeds",
        Default = 20,
        Min = 0,
        Max = 100,
        Rounding = 1,
  ...
  Tabs.Discord:AddParagraph({
        Title = "Join Discord For Update Informatiom",
        Content = "https://novax.serveo.net/discord"
    })
    local CreditsTab = Tabs.Credits
    
 Tabs.Credits:AddParagraph({
        Title = "Auto Parry And Other",
        Content = "Kia_Ainsleyy"
    })
    
    Tabs.Credits:AddParagraph({
        Title = "Helper",
        Content = "Kia_Ainsleyy And Lenny"
    })
   ...
   local ComingTab = Tabs.Coming
   
   Tabs.Coming:AddParagraph({
        Title = "Feedback",
        Content = "Pls Submit Any Of Your Feedback Here Then Press Enter"
    })
    local cum = Tabs.Coming:AddInput("Feedback", {
        Title = "Feedback",
        Default = "Test",
...
ImageButton.Image = "rbxassetid://89103877379195" -- Set the image using the decal ID
...
print("loading succesfull by Nova X") 
